...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('猜数字1～100')
        self.setWindowIcon(QIcon('a1lms-7atzg-002.ico'))
        ...
        self.text = QLineEdit('在这里输数字', self)
        self.text.selectAll()
        self.text.setFocus()
    ...
    def showMessage(self):
        ...
        if guessnumber > self.num:
            QMessageBox.question(self, '看结果', '猜大了！')
            self.text.setFocus()
        elif guessnumber < self.num:
            QMessageBox.question(self, '看结果', '猜小了！')
            self.text.setFocus()
        else:
            QMessageBox.question(self, '看结果', '答对了！进入下一轮！')
            self.num = randint(1, 100)
            self.text.clear()
            self.text.setFocus()
